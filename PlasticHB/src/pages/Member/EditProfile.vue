  ...
  <div class="edit-profile-container">
    ...
    <form v-else @submit.prevent="handleUpdateProfile" class="edit-form">
      ...
      <div class="form-actions">
        ...
        <router-link to="/user" class="cancel-btn">Batal</router-link>
      </div>
    </form>
  </div>
</template>
...
const defaultImage = computed(() => {
  if (user.value.image_profile) {
    ...
    return `http://localhost:3000/uploads/profiles/${user.value.image_profile}`;
  }
...
const handleUpdateProfile = async () => {
  ...
  try {
    ...
    errorMessage.value = err.response?.data?.message || 'Terjadi kesalahan. Silakan coba lagi.'
  }
}
</script>
